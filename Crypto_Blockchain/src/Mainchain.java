...
public class Mainchain {
 ...
 public static HashMap < String, Trans_out > UTXOs = new HashMap < String, Trans_out > (); //list of all unspent transactions
 ...
 public static void main(String[] args) {
  ...
  genesisTransaction.sign_Creator(coinbase.privateKey); //manually sign the genesis transaction
  genesisTransaction.trans_id = "0"; //manually set the transaction id
  genesisTransaction.out.add(new Trans_out(genesisTransaction.amount, genesisTransaction.trans_id, genesisTransaction.receiver)); //manually add the Transactions Output
  ...
  Block genesis = new Block("0", "GenesisBlockStoredData");
  /*
...
add user ID SUPPROT
  ...
  */
  genesis.addNewTransaction(genesisTransaction);	
  addBlock(genesis);

  Block blockone = new Block(genesis.blockHash, "User1IdentityData");
  /*

 add userID SUPPROT;

  */
  blockone.addNewTransaction(walletA.sendFunds(walletB.publicKey, 5000));
  addBlock(blockone);
   ...
   switch (ch) {
    case 1:
     ...
     if (k == 1) {
      ...
      blockarray[i] = new Block(blockchain.get(blockchain.size() - 1).blockHash, "User1IdentityData");
      /*

      USER ID
      */
      blockarray[i].addNewTransaction(walletA.sendFunds(org1.publicKey, ab));
      ...
      USER ID
      */
      blockarray[i].addNewTransaction(walletA.sendFunds(org2.publicKey, ab));
      addBlock(blockarray[i]);
      i++;
      System.out.println("Now Your balance is " + walletA.getBalance());
    ...
    case 2:
     ...
     if (k == 1) {
      ...
      blockarray[i] = new Block(blockchain.get(blockchain.size() - 1).blockHash, "User2IdentityData");
      /*
      ...
      USER ID
      */
      blockarray[i].addNewTransaction(walletB.sendFunds(org1.publicKey, ab));
      addBlock(blockarray[i]);
      i++;
      System.out.println("Now Your balance is " + walletA.getBalance());
      ...
      USER ID
      */
      blockarray[i].addNewTransaction(walletB.sendFunds(org2.publicKey, ab));
      addBlock(blockarray[i]);
      i++;
      System.out.println("Now Your balance is " + walletA.getBalance());
     } else if (k == 3) {
...

     {
      ...
      while (true) {
       ...
       if (k == 1) {
        ...
        if (ok == false) {
         System.out.println("No transaction done by User 2");
        }
 ...
 public static Boolean isChainValid() {
  ...
  HashMap < String, Trans_out > tempUTXOs = new HashMap < String, Trans_out > (); //a temporary working list of unspent transactions at a given block state.
  ...
  for (int i = 1; i < blockchain.size(); i++) {
   ...
   if (!currentBlock.blockHash.equals(currentBlock.computeBlockHash())) {
   ...
   Trans_out tempOutput;
   for (int t = 0; t < currentBlock.transactionList.size(); t++) {
    Transaction currentTransaction = currentBlock.transactionList.get(t);

    if (!currentTransaction.sign_verification()) {
     System.out.println("#Signature on Transaction(" + t + ") is Invalid");
     return false;
    }
    if (currentTransaction.getinpamount() != currentTransaction.getoutamount()) {
     System.out.println("#Inputs are note equal to outputs on Transaction(" + t + ")");
     return false;
    }

    for (Trans_inp input: currentTransaction.inp) {
     tempOutput = tempUTXOs.get(input.trans_out_id);
     ...
     tempUTXOs.remove(input.trans_out_id);
    }

    for (Trans_out output: currentTransaction.out) {
     tempUTXOs.put(output.id, output);
    }

    if (currentTransaction.out.get(0).receiver != currentTransaction.receiver) {
     System.out.println("#Transaction(" + t + ") output reciepient is not who it should be");
     return false;
    }
    if (currentTransaction.out.get(1).receiver != currentTransaction.donor) {
